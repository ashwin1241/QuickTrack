...
public class NewItemFurniture extends AppCompatActivity {
    ...
    private void savenote(){
        ...
        if(newimguri!=null){
                    ...
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()){
                            FirebaseStorage.getInstance().getReference().child(imgname).getDownloadUrl().addOnSuccessListener(task2 -> {
                                ...
                                dbfoodRef.document(a).collection("furRef").add(new Set_item(task2.toString(),title,"Qty available (kg):",initialquantity)).addOnSuccessListener(t2->{
            ...
            CollectionReference dbfoodRef = FirebaseFirestore.getInstance().collection("users");
            dbfoodRef.document(a).collection("furRef").add(new Set_item("",title,"Qty available (kg):",initialquantity));
            Toast.makeText(this, "Item added", Toast.LENGTH_SHORT).show();
            finish();
        }
    }
